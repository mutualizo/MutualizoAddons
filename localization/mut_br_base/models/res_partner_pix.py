...
from ..tools.fiscal import validate_cnpj, validate_cpf
...
class PartnerPix(models.Model):
    ...
    key_type = fields.Selection(selection=KEY_TYPES, string="Type", required=True)
    key = fields.Char(help="PIX Addressing key", required=True)

    partner_bank_id = fields.Many2one(comodel_name="res.partner.bank", string="Bank Account", 
    ...
    def _is_cnpj_or_cpf(self, doc_number):
        res = False
        if validate_cnpj(doc_number):
            res = True
        elif validate_cpf(doc_number):
            res = True
        return res
    ...
    def _normalize_cnpj_cpf(self, doc_number, partner_id):
        ...
        if not self._is_cnpj_or_cpf(doc_number):
            raise ValidationError('Invalid CNPJ/CPF Number!')
        return doc_number
